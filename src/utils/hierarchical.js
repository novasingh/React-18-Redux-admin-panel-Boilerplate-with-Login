...
export function hierarchize(node, parent, nodeProcessor, childrenKey = "children", parentKey = "parent") {
  node[parentKey] = parent;
  nodeProcessor && nodeProcessor(node, parent);
  if (isArray(node[childrenKey])) {
    node[childrenKey].forEach(child => hierarchize(child, node, nodeProcessor, childrenKey, parentKey));
  }
  return node;
}
